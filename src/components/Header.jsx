...
function Header() {
  ...
  // Use effect to hide dropdowns on mouse leave for desktop
  useEffect(() => {
    ...
    <header className="bg-white shadow-md sticky top-0 z-50 ">
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between py-4">
          ...
          {/* Desktop Navigation */}
          <div className="hidden md:flex gap-6 text-gray-700 text-base font-semibold">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                ...
                <div className="absolute left-0 top-[54%] mt-2 bg-white rounded shadow-md px-2 py-2 text-sm w-48">
                  <ul>
                    {categoryItems.map((loan, index) => (
                      <li key={index} className="hover:bg-blue-50 px-3 py-1 rounded">
                        <Link to="#">{loan}</Link>
                      </li>
                    ))}
                  </ul>
            ...
            <div
              className="relative"
              onMouseEnter={() => setShowInsurance(true)}
              onMouseLeave={() => setShowInsurance(false)}
            >
              <Link
                to="/insurance"
      ...
      <div
        ...
        className={`md:hidden fixed top-16 right-0 h-full w-screen max-w-sm bg-white shadow-lg z-40 transform transition-transform duration-300 ${
